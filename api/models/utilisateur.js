...
class Utilisateur {
    ...
    static async createUser({nom, prenom, email, mot_de_passe}) {
        const result = await pool.query(
            'INSERT INTO utilisateur (nom, prenom, email, mot_de_passe) VALUES ($1, $2, $3, $4) RETURNING *',
            [nom, prenom, email, mot_de_passe]
        )
        return result.rows[0]
    }

    static async updateUser(id_utilisateur, {nom, prenom, email, mot_de_passe}) {
        const result = await pool.query(
            'UPDATE utilisateur SET nom = $1, prenom = $2, email = $3, mot_de_passe = $4 WHERE id_utilisateur = $5 RETURNING *',
            [nom, prenom, email, mot_de_passe, id_utilisateur]
        );
    }
